...
public class ImageSelectorActivity extends AppCompatActivity {
    ...
    private ActivityResultLauncher activityResultLauncher = registerForActivityResult(new ActivityResultContracts.RequestPermission(), isGranted -> {
        if (isGranted){
    ...
    private ActivityResultLauncher cameraActivityResultLauncher = registerForActivityResult(new ActivityResultContracts.RequestPermission(), isGranted -> {
        if (isGranted){
...
/*        if (Build.VERSION.SDK_INT >= 33 && getApplicationInfo().targetSdkVersion >= 33){
             ...
             * 这个判断在手机系统不是13或者targetSdk小于33时适用
             */
//        if (captureIntent.resolveActivity(getPackageManager()) != null) {
            File photoFile = null;
            Uri photoUri = null;
